...
class Tool:
    ...
    runner: Callable[[Dict[str, Any]], str] = None  # 工具执行函数

    def __post_init__(self):
        # 兼容旧的只传 name 的用法
        if self.runner is None and callable(self.name):
            # 旧用法：Tool(callable_func, "description")
            # name 参数实际是 callable
            self.runner = self.name
            self.name = self.runner.__name__ if hasattr(self.runner, '__name__') else "unnamed"
    ...
    def execute(self, arguments: Dict[str, Any]) -> str:
        ...
        if self.runner is None:
            return f"Tool {self.name} has no runner function"
